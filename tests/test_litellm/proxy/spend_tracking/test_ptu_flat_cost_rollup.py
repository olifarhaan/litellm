...
class _FakeSentinelTable:
    ...
    async def find_many(self, where=None):
        ...
        return [
            _stored_sentinel_row(team_id, day, model)
            for (team_id, day, api_key, model) in self.rows
            if api_key == PTU_SENTINEL_API_KEY and (not bounds or bounds["gte"] <= day <= bounds["lte"])
        ]

    def seed(self, team_id, day, model, flat_cost, updated_at=None):
        self.rows[(team_id, day.isoformat(), PTU_SENTINEL_API_KEY, model)] = {
            "ptu_flat_cost": flat_cost,
            "updated_at": updated_at or datetime.now(timezone.utc),
        }


def _stored_sentinel_row(team_id, day, model):
    row = MagicMock()
    row.team_id = team_id
    row.date = day
    row.model = model
...
async def test_backfill_prices_a_day_the_daily_run_missed():
    ...
    table = _FakeSentinelTable()
    table.seed("t", _day(-2), "gpt-4o-mini-ptu", 240.0)
    table.seed("t", _day(0), "gpt-4o-mini-ptu", 240.0)
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-2)))], table)

    result = await run_ptu_flat_cost_backfill(prisma, today=TODAY)

    assert result.rows_written == 1
    assert table.upsert_keys == [("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o-mini-ptu")]
...
async def test_backfill_prices_the_partial_first_day_by_active_hours():
    ...
    first_day = table.rows[("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o-mini-ptu")]
    assert first_day["ptu_flat_cost"] == pytest.approx(10 * (15 + 59 / 60))
    assert table.rows[("t", _day(0).isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o-mini-ptu")][
        "ptu_flat_cost"
    ] == pytest.approx(240.0)
...
async def test_backfill_leaves_an_existing_row_untouched_when_config_changed():
    ...
    table = _FakeSentinelTable()
    table.seed("t", _day(-1), "gpt-4o-mini-ptu", 240.0)
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-1)), cost_per_ptu_per_hour=5.0)], table)

    result = await run_ptu_flat_cost_backfill(prisma, today=TODAY)

    already_priced = ("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o-mini-ptu")
...
@pytest.mark.asyncio
async def test_backfill_does_not_touch_a_row_whose_model_was_since_deleted():
    """Routing the catch-up pass through the single-day rollup would inherit its prune and
    delete charges for a deployment that has since been removed."""
    table = _FakeSentinelTable()
    table.seed("t", _day(-1), "retired-model", 480.0, updated_at=datetime(2020, 1, 1, tzinfo=timezone.utc))
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-1)), model_name="live-model")], table)

    await run_ptu_flat_cost_backfill(prisma, today=TODAY)

    assert ("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "retired-model") in table.rows
    assert table.delete_many_calls == []


@pytest.mark.asyncio
async def test_backfill_never_calls_delete_many():
    """Deciding a sentinel row is stale stays the single-day rollup's job."""
    table = _FakeSentinelTable()
    table.seed("t", _day(-3), "stale", 1.0, updated_at=datetime(2020, 1, 1, tzinfo=timezone.utc))
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-3)))], table)

    await run_ptu_flat_cost_backfill(prisma, today=TODAY)

    assert table.delete_many_calls == []
    assert ("t", _day(-3).isoformat(), PTU_SENTINEL_API_KEY, "stale") in table.rows
...
async def test_backfill_fills_a_second_model_on_a_day_that_already_has_a_row():
    ...
    table = _FakeSentinelTable()
    table.seed("t", _day(-1), "model-a", 240.0)
    prisma = _prisma_for(
        [
    ...
    assert ("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "model-b") in table.upsert_keys
    assert ("t", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "model-a") not in table.upsert_keys


@pytest.mark.asyncio
async def test_backfill_fills_a_second_team_on_a_day_that_already_has_a_row():
    table = _FakeSentinelTable()
    table.seed("team-1", _day(-1), "shared-name", 240.0)
    prisma = _prisma_for(
        [
    ...
    assert ("team-2", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "shared-name") in table.upsert_keys
    assert ("team-1", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "shared-name") not in table.upsert_keys
...
async def test_backfill_keys_gaps_on_the_public_model_name():
    ...
    table = _FakeSentinelTable()
    table.seed("team_x", _day(-1), "gpt-4o", 240.0)
    ...
    assert ("team_x", _day(-1).isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o") not in table.upsert_keys
    assert result.rows_written == 1
...
async def test_backfill_is_a_no_op_on_a_fully_priced_range():
    table = _FakeSentinelTable()
    for offset in (-2, -1, 0):
        table.seed("t", _day(offset), "gpt-4o-mini-ptu", 240.0)
    prisma = _prisma_for([_windowed_row(effective_from=_midnight(_day(-2)))], table)
...
async def test_scheduled_rollup_backfills_after_pricing_the_day():
    ...
    yesterday_key = ("t", yesterday.isoformat(), PTU_SENTINEL_API_KEY, "gpt-4o-mini-ptu")
